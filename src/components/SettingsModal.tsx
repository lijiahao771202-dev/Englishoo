...
import { X, RotateCcw, Save, Database, Palette, Loader2, BrainCircuit, Key, Volume2, Keyboard, Smile, UploadCloud, DownloadCloud, Image as ImageIcon, Trash2, Upload, Book } from 'lucide-react';
                  ...
                  {isImporting ? (
                    <div className="space-y-3">
                      <div className="flex justify-between text-xs text-white/70">
                        <span>正在处理: {importProgress.word || '准备中...'}</span>
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  setIsImporting(true);
                                  // Reset progress
                                  setImportProgress({ total: 1, current: 0, word: '准备中...' });
                        ...
                        <button
                          onClick={() => {
                            ...
                            input.onchange = (e) => {
                              const file = (e.target as HTMLInputElement).files?.[0];
                              if (!file) return;
                              // Re-use the existing logic if possible, or just call importCustomDeck directly
                              // Or we can re-use handleCustomImport if we pass a name?
                              // Let's just use a simple prompt for name here or default to filename
                              const name = file.name.replace('.json', '').replace('.txt', '');
                              handleCustomImport(name, URL.createObjectURL(file));
                            };
                            input.click();
                          }}
