...
export function DeckStatistics({ cards, logs, className }: DeckStatisticsProps) {
  ...
  const calendarData = useMemo(() => {
    ...
    // Map logs to days
    const dailyStats = new Map<string, { learned: number; reviewed: number }>();
    
    globalStats.deckLogs.forEach(log => {
        const dayKey = format(new Date(log.review), 'yyyy-MM-dd');
        const current = dailyStats.get(dayKey) || { learned: 0, reviewed: 0 };
        
        // Check if it was a learning event (State.New -> something else)
        // Note: ts-fsrs log.state is the state BEFORE review.
  ...
  }, [calendarDate, globalStats.deckLogs]);

  // --- 4. Selected Date Stats ---
  const selectedDateStats = useMemo(() => {
    if (!selectedDate) return null;
    const key = format(selectedDate, 'yyyy-MM-dd');
    return calendarData.dailyStats.get(key) || { learned: 0, reviewed: 0 };
