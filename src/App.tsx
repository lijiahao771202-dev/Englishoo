...
function App() {
  ...
  const handleStartSession = async (limits: { newLimit: number; reviewLimit: number; newGroupLimit?: number }) => {
      ...
      try {
          ...
          if (limits.newGroupLimit && limits.newGroupLimit > 0 && currentDeckId) {
              // Use cached clusters for performance and consistency
              // We need all cards for the deck to get proper clusters
              ...
              // Filter clusters that have new cards
              const newCardIds = new Set(newCards.map(c => c.id));
              const clustersWithNewWords = clusters.filter(c => 
                  c.items.some(item => newCardIds.has(item.id))
              );

              // Sort by number of new cards in the cluster (optional, or just take first)
              // Let's sort to maximize learning efficiency? Or keep semantic order?
              // Keeping original order from getDeckClusters (usually by size) is fine, 
              // but we might want to prioritize clusters that are "mostly new" or "mostly ready"?
              // Let's just take the first N that have new cards.
              
              groups = clustersWithNewWords.slice(0, limits.newGroupLimit);
              
              // Queue should only contain NEW cards from these groups?
              // Or should we learn the whole group?
              // User said: "learn by groups". Usually implies learning the content of the group.
              // If words are already learned, maybe we skip them in the queue but show in graph.
              // Let's filter queue to only new cards to avoid forcing review of known words in "Learn" mode.
              const groupItems = groups.flatMap(c => c.items);
              queue = groupItems.filter(item => newCardIds.has(item.id));
              
              if (queue.length === 0 && newCards.length > 0) {
          ...
          // 2. New Words (Legacy Limit Mode)
               ...
               queue = [...newCards.slice(0, limits.newLimit)];
          }
  ...
  const handleStartClusterSession = (groups: Array<{ label: string; items: WordCard[] }>) => {
  ...
  const handleUpdateCard = async (updatedCard: WordCard) => {
      ...
      setDueCards(prev => prev.map(c => c.id === updatedCard.id ? updatedCard : c));
  ...
  const handleGenerateMindMap = async (card: WordCard) => {
    if (!apiKey) {
        alert("请先在设置中配置 DeepSeek API Key。");
        return undefined;
    }
    try {
        const mindMap = await generateMindMap(card.word, apiKey);
        const updatedCard = { ...card, mindMap };
        await handleUpdateCard(updatedCard);
        return updatedCard;
    } catch (e) {
        alert("生成思维导图失败");
  ...
  const handleSaveMindMap = async (card: WordCard, mindMapData: NonNullable<WordCard['mindMap']>) => {
    const updatedCard = { ...card, mindMap: mindMapData };
    await handleUpdateCard(updatedCard);
    return updatedCard;
  };
        ...
        <AnimatePresence mode="wait">
            ...
            {view === 'learn' && (
                ...
                >
                    ...
                    <span className="text-sm opacity-60">本次学习 {sessionQueue.length} 个单词</span>
                    </div>
